...
- Для перетаскивания на мобильных нужно удерживать 0.3 сек
...
1. **Убедитесь, что удерживаете достаточно долго** (0.3 сек)
2. **Не двигайте палец во время удержания** - подождите, пока карточка "прилипнет"
3. **Обновите страницу** - иногда помогает перезагрузка
4. **Проверьте, что JavaScript включен** в настройках Safari
5. **Попробуйте в режиме инкогнито** - расширения могут блокировать touch events

### Технические детали iOS совместимости

- Используется `touch-action: none` для предотвращения конфликтов со скроллингом
- Event listeners с `passive: false` для iOS Safari
- Увеличенный threshold движения (15px) для устойчивости на iOS
